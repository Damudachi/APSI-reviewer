...
## 🏛️ 1. The Core Architecture Rule: Layering
...
## 🗄️ 2. PostgreSQL & `pg` Database Layer
...
## 🌐 3. Express REST API & Middleware Pipeline

### Blank `createApp(pool)` 5-Step Boilerplate
When starting a blank `createApp` function in the exam:
...
| **204** | **No Content** | Deleted successfully | Successful `DELETE` (no body, `res.sendStatus(204)`). |
...
## 🛑 6. Top 5 Syntax Pitfalls to Avoid

1. ⚠️ **Forgetting `await`**:
   Repository calls return Promises. Always write `await repo.functionName(...)`.
2. ⚠️ **Forgetting `return` before error responses**:
   Write `return res.status(400)...` so code execution stops immediately.
3. ⚠️ **`typeof` vs `Number.isInteger()` vs `.trim()`**:
   * Use `typeof name === 'string'` for text strings.
   * Use `Number.isInteger(count)` for whole numbers.
   * `.trim()` is ONLY for strings! Calling `(42).trim()` will crash your server!
4. ⚠️ **Missing leading spaces in SQL concatenation**:
   `query += ' WHERE b.days >= $1'` (always include a space before `WHERE` and `ORDER BY`).
5. ⚠️ **Sending objects instead of arrays**:
   `res.status(200).json(items)` sends `[...]`. Don't wrap in extra object `{ items }` unless explicitly requested.
